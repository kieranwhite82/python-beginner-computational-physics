...
def f1(): raise Exception("Invalid number/character entered for integrand \
selection.")

def f2(): raise Exception("Invalid number/character entered for area \
calculation type.")

def f3(): raise Exception("The user entered limits were numerically equal, \
therefore the integral is equal to zero.")
...
fI = eval(input("Enter the number corresponding to the integrand from the \
option list to numerically integrate: "))  # noqa
if fI > 0 and fI < 4 and (fI % 2 == 0 or fI % 2 == 1):
    print("Integrand {} selected.".format(fI))
elif fI == 4:
    userequ = input("Enter a function of x (using numpy mathematical \
functions, using numpy as np, for example: np.exp(x); the same applies to \
special limits, for example: np.pi): ")
else:
    f1()
...
if Is == 1:
    ...
    if fI == 1:
        def y(x):
            return abs(np.power(1 + np.power(x, 2, dtype=float), -1, \
                                dtype=float))
    elif fI == 2:
        def y(x):
...
if b == a:
    ...
    a = d
    b = c
    print("User entered limits are reversed in the case of the upper limit \
    being numerically smaller than the lower limit.")
else:
...
print("The accuracy {} to iterate to was reached with the numerical integral \
as {} with {} integration steps.".format(E, FRRn, NRR))
...
print("The accuracy {} to iterate to was reached with the numerical integral \
as {} with {} integration steps.".format(E, FTRn, NTR))
